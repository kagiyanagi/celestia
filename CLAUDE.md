...
- **Postgres** (when `DATABASE_URL` is set) — normalized tables: `users` (slim profile/prefs/auth row), `sessions`, `stream_mappings`, and **`library_entries` (one row per user+anime)** and **`history_entries` (one row per user+anime+episode, capped per user)**. Schema auto-created on first use, with one-time migrations from the legacy `app_state` blob and from any library/history still embedded in a user's payload.
- **JSON file** (`data/app-db.json`) — development fallback only. Mirrors the same shape (top-level `libraryEntries`/`historyEntries` collections tagged by `userId`); hoists any legacy embedded arrays on read. It does **not** persist on serverless hosts, so production needs `DATABASE_URL`.

The split is deliberate: tracking data is **never** stored on the user row, so a tracking write touches a single entry row (not a multi-MB blob) and the hot auth read never transfers it — see Tracking & sync. Higher-level domain stores wrap `getStore()`: `account-store.ts` (profile, library, history), `stream-mapping-store.ts` (verified stream matches).

### Tracking & sync (library + history)
Library and watch history are the durable product value (not streaming). They live in **their own normalized tables** (`library_entries` / `history_entries`), one row per entry — **not** inline on the user record. Every tracking write goes through an `account-store.ts` helper that performs a **single scoped row write** (`upsertLibraryEntry`, `recordHistory`, …), never a whole-user rewrite; reads are scoped too (`getLibraryEntries` / `getHistoryEntries` / `getLibraryEntry`). The stored `UserRecord` is slim; the client-facing `PublicUser` is the slim user **with** library/history assembled in, produced only where the client needs the full view (`getSessionPublicUser` for the root layout + `/api/auth/session`, and the mutation endpoints whose response the client folds into context). The hot `getSessionUser()` path returns the slim `SessionUser` and transfers no tracking data. A `LibraryEntry` carries status (`planning`/`watching`/`on_hold`/`dropped`/`completed`/`rewatching`), score/progress/repeat/notes/dates, `addedAt` (notification window anchor — see Notifications), and `aniListEntryId` (the link to its AniList list row).
...
### Auth & sessions
`src/lib/auth.ts` owns cookie sessions (`celestia_session`), scrypt password hashing, guest accounts, and device tracking. `requireSessionUser()` / `getSessionUser()` gate API routes; they return a **redacted, slim `SessionUser`** (no library/history, never the raw `UserRecord` secrets), so routine auth never transfers tracking data. `getSessionPublicUser()` assembles the full `PublicUser` (slim user + library + history) for the client bootstrap. Session IDs are regenerated on privilege escalation (e.g. AniList OAuth link). Stored OAuth tokens are encrypted at rest by `src/lib/crypto.ts` (AES-256-GCM keyed off `APP_SECRET`; self-describing `enc:v1:` prefix; dev falls back to a machine-local secret).
